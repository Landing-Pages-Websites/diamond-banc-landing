...
import { R_HOW_IT_WORKS, ROLEX_CTA_LABEL } from "@/lib/rolex-content";

export function RolexHowItWorks(): React.ReactElement {
  return (
    <section id="how-it-works" className="bg-[var(--color-ink)] py-20 text-white md:py-28">
      <div className="mx-auto max-w-6xl px-5 md:px-8">
        <Reveal className="max-w-2xl">
          <p className="eyebrow text-[var(--color-teal-400)]">{R_HOW_IT_WORKS.eyebrow}</p>
          <h2 className="mt-3 font-display text-[2.25rem] leading-[1.08] text-white md:text-5xl">
            {R_HOW_IT_WORKS.headline}
          </h2>
        </Reveal>

        <ol className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {R_HOW_IT_WORKS.steps.map((step, i) => (
